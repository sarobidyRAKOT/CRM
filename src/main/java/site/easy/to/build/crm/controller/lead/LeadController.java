...
import site.easy.to.build.crm.service.lead.Depense_LeadService;
import site.easy.to.build.crm.service.lead.LeadActionService;
import site.easy.to.build.crm.service.lead.LeadService;
import site.easy.to.build.crm.service.lead.Lead_supService;
import site.easy.to.build.crm.service.lead.Montant_dlService;
...
public class LeadController {
    ...
    private final Lead_supService lead_supService;
    private final Depense_LeadService depense_LeadService;
    private final Montant_dlService montant_dlService;

    @Autowired
    public LeadController(Depense_LeadService depense_LeadService, LeadService leadService, AuthenticationUtils authenticationUtils, UserService userService, CustomerService customerService,
        ...
        Lead_supService lead_supService, Montant_dlService montant_dlService
    ) {
        ...
        this.lead_supService = lead_supService;
    ...
    public String showDetails(@PathVariable("id") int id, Model model, Authentication authentication) {
        ...
        // mila maka depense reetres (na ray na firy)

        Depense_lead depense_lead = depense_LeadService.findByLeadLeadId(lead.getLeadId());
        List <Montant_dl> montant_dls = null;
        
        if (depense_lead != null) {
            montant_dls = montant_dlService.findAll();
        }




        model.addAttribute("depense_lead", depense_lead);
        model.addAttribute("montant_dls", montant_dls);
    ...
    public String deleteLead(@PathVariable("id") int id, Authentication authentication) {
        ...
        if (! AuthorizationUtil.checkIfUserAuthorized(employee, loggedInUser)) {
            return "error/access-denied"; // pas d'acces pour supprimer ce lead ...
        }

        Lead_sup lead_sup = new Lead_sup();
        lead_sup.setLead(lead);
        lead_supService.save(lead_sup);
